...
@description('The queue names to pre-create. Used for triggering/responding in the integration.')
...
module api './app/api.bicep' = {
  ...
  params: {
    ...
    appSettings: {
      AZURE_AI_PROJECT_CONNECTION_STRING: aiProject.outputs.projectConnectionString
      // This shouldn't be required: STORAGE_QUEUES_CONNECTION__serviceUri: 'https://${storage.outputs.name}.queue.${environment().suffixes.storage}'
      STORAGE_QUEUES_CONNECTION__queueServiceUri: 'https://${storage.outputs.name}.queue.${environment().suffixes.storage}'
    }
...
module storage 'core/storage/storage-account.bicep' = {
  ...
  params: {
    ...
    queueNamesToCreate: queueNamesToCreate
...
output AZURE_AI_PROJECT_CONNECTION_STRING string = aiProject.outputs.projectConnectionString
output STORAGE_QUEUES_CONNECTION__queueServiceUri string = 'https://${storage.outputs.name}.queue.${environment().suffixes.storage}'
